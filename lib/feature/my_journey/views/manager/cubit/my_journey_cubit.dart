...
class MyJourneyCubit extends Cubit<MyJourneyState> {
  ...
  Future<void> fetchJourneys({String? search}) async {
    emit(MyJourneyLoading());
    final result = await MyJourneyDataSource.getJourneys(search: search);
    result.fold(
      (failure) => emit(MyJourneyError(failure.errMessage)),
      (journeyModel) {
