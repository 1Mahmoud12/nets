...
class AppVersionChecker {
  ...
  AppVersionChecker({this.currentVersion, this.appId, this.androidStore = AndroidStore.googlePlayStore});
    ...
    if (Platform.isAndroid) {
      switch (androidStore) {
        case AndroidStore.apkPure:
          return _checkApkPureStore(_currentVersion, _packageName);
        default:
          return _checkPlayStore(_currentVersion, _packageName);
      }
    } else if (Platform.isIOS) {
      return _checkAppleStore(_currentVersion, _packageName);
    } else {
      return AppCheckerResult(_currentVersion, null, '', 'The target platform "${Platform.operatingSystem}" is not yet supported by this package.');
    }
  }
  ...
  Future<AppCheckerResult> _checkAppleStore(String currentVersion, String packageName) async {
    ...
    String? newVersion;
    String? url;
    final uri = Uri.https('itunes.apple.com', '/lookup', {'bundleId': packageName, 'timestamp': '${DateTime.now().millisecondsSinceEpoch}'});
    try {
  ...
  Future<AppCheckerResult> _checkPlayStore(String currentVersion, String packageName) async {
    ...
    String? newVersion;
    String? url;
    final uri = Uri.https('play.google.com', '/store/apps/details', {'id': packageName, 'timestamp': '${DateTime.now().millisecondsSinceEpoch}'});
    try {
      final response = await http.get(uri);
      if (response.statusCode != 200) {
...
Future<AppCheckerResult> _checkApkPureStore(String currentVersion, String packageName) async {
  ...
  String? newVersion;
  String? url;
  final Uri uri = Uri.https('apkpure.com', '$packageName/$packageName', {'timestamp': '${DateTime.now().millisecondsSinceEpoch}'});
  try {
    final response = await http.get(uri);
    if (response.statusCode != 200) {
      ...
      newVersion =
          RegExp(r'<div class="details-sdk"><span itemprop="version">(.*?)<\/span>for Android<\/div>').firstMatch(response.body)!.group(1)!.trim();
      url = uri.toString();
    }
  } catch (e) {
    errorMsg = '$e';
